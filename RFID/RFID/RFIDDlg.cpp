...
IMPLEMENT_DYNAMIC(CRFIDDlg, CDialogEx);
...
BEGIN_MESSAGE_MAP(CRFIDDlg, CDialogEx)
	...
	ON_WM_PAINT()
	ON_WM_QUERYDRAGICON()
	ON_COMMAND(ID_MENU_FIND, &CRFIDDlg::OnMenuFind)
END_MESSAGE_MAP()
...
BOOL CRFIDDlg::OnInitDialog()
{
	...
	//查找标签菜单项
	dialog_Find.Create(IDD_DIALOG_FIND);
	dialog_Find.ShowWindow(SW_SHOW);
...
void CRFIDDlg::OnCancel()
{
	if (CanExit())
		CDialogEx::OnCancel();
}

BOOL CRFIDDlg::CanExit()
{
	// 如果代理对象仍保留在那里，则自动化
	//  控制器仍会保持此应用程序。
